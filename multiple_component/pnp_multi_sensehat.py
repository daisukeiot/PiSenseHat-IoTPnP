...
# The device "Pi_SenseHat" that is getting implemented using the above interfaces.
...
async def reboot_handler(values):
    if values == 9999:    
     #### for Demo please use below ###    
        #print(
        #    "Will Reboot now"
        #    )
        #os.popen('sudo reboot now')

    #### for cetification please use below --  5 mins delay ###
        print(
            "Will Reboot now after 5 mins"
            )
        os.popen('sudo shutdown -r +5')
...
async def main():
    ...
    properties_root = pnp_helper.create_reported_properties( manufacturer = "Pi Fundation")
    properties_RaspberryPi = pnp_helper.create_reported_properties( component_name_1, manufacturer = "OKdo")
    properties_SenseHat = pnp_helper.create_reported_properties( component_name_2, manufacturer = "Pi Fundation")
